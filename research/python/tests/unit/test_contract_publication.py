...
def _contracts() -> tuple[FeatureSchema, ModelArtifact, Forecast]:
    schema = FeatureSchema(
        schema_version="v1", feature_names=["return_1"], dtypes={"return_1": "float"},
        normalization={}, lookback_periods=1, source_requirements=["bars"], feature_hash="schema-hash",
    )
    artifact = ModelArtifact(
        artifact_id="artifact-1", model_id="btc-model", model_type="lightgbm", model_version="v1",
        feature_schema_hash="schema-hash", dataset_hash="dataset-hash", training_window={}, parameters={},
        random_seed=42, metrics={}, evidence_grade="B_Close", evidence_profile=EvidenceProfile(
            evidence_id="evidence-1", economic_hypothesis="test hypothesis",
            counter_hypothesis="test counter-hypothesis", primary_evidence_ids=["source-1"],
            transfer_grade="B_Close", transfer_reason="test evidence",
        ), validation_gates=sorted(REQUIRED_EXECUTION_GATES), support_domain={}, git_commit="test",
        config_hash="config", creation_timestamp=datetime.now(UTC), artifact_hash="artifact-hash",
    )
    forecast = Forecast(
        expert_id="btc-model", model_id="btc-model", model_version="v1", instrument="BTC/USD",
        as_of_time=datetime.now(UTC), forecast_family="directional_return_bps", horizon_minutes=5,
        point_forecast=12.0, confidence=0.8, calibration_status="validated",
        support_domain_status="in_domain", feature_schema_hash="schema-hash", artifact_hash="artifact-hash",
        status="valid",
    )
...
def test_publishes_complete_bundle_before_marking_model_validated(tmp_path: Path):
    ...
    ContractPublisher(root, registry).publish_validated(schema, artifact, forecast, root / "model.json")
