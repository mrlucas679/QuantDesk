...
class TransactionCostModel(ABC):
    @abstractmethod
    def calculate_cost(self, amount: float, price: float, **kwargs) -> float:
...
class OptionsCostModel(TransactionCostModel):
    def __init__(self, fee_per_contract: float = 0.65, slippage_bps: float = 5.0):
...
class CryptoCostModel(TransactionCostModel):
    ...
    def calculate_cost(
        self, amount: float, price: float, is_maker: bool = False, **kwargs
    ) -> float:
        notional = abs(amount * price)
